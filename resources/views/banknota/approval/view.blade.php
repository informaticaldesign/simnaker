...
<div class="row mb-2">
    ...
    <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item">Approval Bank Nota 1</li>
        </ol>
    </div>
</div>
@stop
...
<div class="row">
    <div class="col-12">
        <div class="invoice p-3 mb-3" style="width: 29.7cm;">
            ...
            <div class="reason">
                Alasan Ditolak:
                <textarea id="reason" name="reason" rows="5" cols="10" class="form-control"></textarea>
                <div class="invalid-feedback invalid-reason"></div>
            </div>
        </div>
        <div class="d-grid gap-2 d-md-flex justify-content-md-center pb-2">
            <a href="{{ url('/banknota/approval') }}" class="btn btn-default float-right"><i class="fa fa-close"></i>&nbsp;Tutup</a>&nbsp;
            <a href="javascript:void(0)" data-toggle="tooltip" data-id="{{ $banknota->uuid }}" data-original-title="Cetak" class="action-cetak btn btn-warning"><i class="fas fa-print"></i>&nbsp;Cetak</a>&nbsp;
            <a href="javascript:void(0)" data-toggle="tooltip" data-id="{{ $banknota->uuid }}" data-original-title="Cetak" class="action-approve btn btn-success"><i class="fas fa-check"></i>&nbsp;Disetujui</a>&nbsp;
            <a href="javascript:void(0)" data-toggle="tooltip" data-id="{{ $banknota->uuid }}" data-original-title="Cetak" class="action-reject btn btn-danger"><i class="fa fa-remove"></i>&nbsp;Ditolak</a>
        </div>
    </div>
</div>
...
<script>
    $(function () {
        ...
        }).on('click', 'a.action-approve', function (e) {
            e.preventDefault();
            $('a.action-approve').html('<i class="fa fa-spinner fa-spin"></i> Processing...');
            $('a.action-approve').prop('disabled', true);
            e.preventDefault();
            var _id = $('a.action-approve').attr('data-id');
            $.ajax({
                url: "{{ url('banknota/approval/send') }}",
                type: "POST",
                data: {
                    id: _id,
                    status: 'approve'
                ...
                success: function (result) {
                    if (result.success) {
                        window.location.href = "{{ url('banknota/approval')}}";
                    } else {
                        Swal.fire({
                            title: 'Gagal',
                            text: "Description",
                            ...
                            $('a.action-approve').html('Disetujui&nbsp;<i class="fas fa-paper-plane"></i>');
                            $('a.action-approve').prop('disabled', false);
                        });
                    }
                },
                error: function (err) {
        ...
        }).on('click', '.action-reject', function (e) {
            e.preventDefault();
            $('div.reason').show();
            $('a.action-reject').html('<i class="fa fa-spinner fa-spin"></i> Processing...');
            $('a.action-reject').prop('disabled', true);
            e.preventDefault();
            var _id = $('a.action-reject').attr('data-id');
            $.ajax({
                url: "{{ url('banknota/approval/send') }}",
                type: "POST",
                data: {
                ...
                success: function (result) {
                    if (result.success) {
                        ...
                        Swal.fire({
                            ...
                            $('a.action-reject').html('Ditolak&nbsp;<i class="fas fa-paper-plane"></i>');
                            $('a.action-reject').prop('disabled', false);
                        });
                    }
                },
                error: function (err) {
