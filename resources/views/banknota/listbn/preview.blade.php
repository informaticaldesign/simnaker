    ...
    <div class="col-12">
        <div class="invoice p-3 mb-3" style="width: 29.7cm;">
            ...
            <table>
                ...
                <tr>
                    <td style="text-align:left;vertical-align:top;padding-right: 10; width:50px;"><a href='#' class="btn-edit-pasal" data-id="{{ $val->id }}"><i class="fa fa-edit"></i></a> <b>{{ $i }}.</b> </td>
        ...
        <div class="d-grid gap-2 d-md-flex justify-content-md-center pb-2">
            <a href="{{ url('/banknota/listbn/'.$banknota->uuid.'/edit') }}" class="btn btn-danger float-right"><i class="fa fa-arrow-left"></i>&nbsp;Kembali</a>&nbsp;
            <a href="{{ url('/banknota/listbn') }}" class="btn btn-success"><i class="fa fa-save"></i>&nbsp;Simpan</a>&nbsp;
...
$(function () {
    ...
    $(document).on("click", "button.btn-add-jenis-bank-nota", function (e) {
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.success) {
                    window.location.href = "{{ url('banknota/listbn')}}";
                } else {
                    Swal.fire({
                        title: 'Gagal',
                        text: "Description",
    ...
    $('body').on('click', 'a.btn-edit-pasal', function () {
        $('#ajaxAddModel').modal('show');
        var _id = $(this).attr('data-id');
        $('input#id-description').val(_id);
        $('#summernote-description').summernote("code", $('td.desc-banknota-' + _id).html());
    });
});
</script>
@stop