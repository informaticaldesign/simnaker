    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <input type="hidden" name="id" value="{{ $banknota->id }}">
                <div class="row">
                    <div class="col-sm-12">
                        <div class="row">
                            <div class="col-sm-6">
                                <div class="mb-1">
                                    ...
                                    <div class="col-sm-12">
                                        <input type="text" class="form-control" name="document_no" id="document_no" value="{{ $banknota->document_no }}" readonly>
                                        <div class="invalid-feedback invalid-document_no"></div>
                                    </div>
                                </div>
                                <div class="mb-1">
                                    <label for="spt_id" class="col-sm-12 col-form-label">Nomor SPT <span class="text-danger">*</span></label>
                                    <div class="col-sm-12">
                                        <select class="form-control status" id="spt-id" name="spt_id" disabled>
                                            <option value="" selected disabled>Pilih Nomor SPT</option>
                                            @foreach($spt as $key => $val)
                                            <option value="{{ $key }}" {{ ($banknota->spt_id == $key)?'selected':'' }}>{{ $val }}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="mb-1">
                                    ...
                                    <div class="col-sm-12">
                                        <select class="form-control" name="sifat_id">
                                            ...
                                            <option value="{{ $key }}" {{ ($banknota->sifat_id == $key)?'selected':'' }}>{{ $val }}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="mb-1">
                                    ...
                                    <div class="col-sm-12">
                                        <select class="form-control" name="kota">
                                            ...
                                            <option value="{{ $val }}" {{ ($banknota->kota == $key)?'selected':'' }}>{{ $val }}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="mb-1">
                                    ...
                                    <div class="col-sm-12">
                                        <input type="text" class="form-control" name="perihal" id="perihal" value="{{ $banknota->perihal }}">
                                        <div class="invalid-feedback invalid-perihal"></div>
                                    </div>
                                </div>
                                <div class="mb-1">
                                    <label for="tanggal" class="col-sm-6 col-form-label">Tanggal Bank Nota<span class="text-danger">*</span></label>
                                    <div class="col-sm-6">
                                        <input type="text" class="form-control" name="tanggal" id="tanggal" value="{{ $banknota->tanggal }}">
                                        <div class="invalid-feedback invalid-tanggal"></div>
                                    </div>
                                </div>
                                <div class="mb-1">
                                    <label for="jenis_id" class="col-sm-12 col-form-label">Jenis Bank Nota<span class="text-danger">*</span></label>
                                    <div class="col-sm-12 col-jenis-bank-nota" data-id="{{ $relbank->count() }}">
                                        <?php $_no = 0; ?>
                                        @foreach($relbank as $key => $valrel)
                                        <div class="row row-jenis-bank-nota mt-1 row-{{ $_no }}">
                                            <div class="col-10">
                                                <select class="form-control" name="jenis_id[{{ $_no }}]">
                                                    <option value="">Pilih Jenis Bank Nota</option>
                                                    @foreach($jenis as $key => $val)
                                                    <option value="{{ $key }}" {{ $valrel->jenis_id==$key?'selected':'' }}>{{ $val }}</option>
                                                    @endforeach
                                                </select>
                                                <div class="invalid-feedback invalid-jenis_id.{{ $_no }}"></div>
                                            </div>
                                            <div class="col-2">
                                                <button class="btn btn-danger btn-remove-jenis-bank-nota" data-value="{{ $_no }}"><i class="fa fa-trash"></i></button>
                                            </div>
                                        </div>
                                        <?php $_no++; ?>
                                        @endforeach
                                        <div class="row row-jenis-bank-nota mt-1 row-{{ $_no }}"></div>
                                    </div>
                                    <div class="col-12 mt-1">
                                        <button class="btn btn-success btn-add-jenis-bank-nota"><i class="fa fa-plus ml"></i> Tambah Jenis Bank Nota</button>
                                    </div>
                                </div>
                            </div>
                            <div class="col-sm-6">
                                <div class="">
                                    <h6 id="company-label">Nama Perusahaan : </h6>
                                    <h2 id="company-name">{{ $company->name }}</h2>
                                    <p id="company-address" class="lead">Alamat: {{ $company->address }}</p>
                                    <p id="company-phone" class="lead">Phone: {{ $company->phone }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="card-footer">
            <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                <a href="{{ url('/banknota/listbn') }}" class="btn btn-danger float-right"><i class="far fa-window-close"></i>&nbsp;Batal</a>&nbsp;
    ...
    $('button.btn-action-save').click(function (e) {
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.success) {
                    window.location.href = "{{ url('banknota/listbn/preview')}}" + "/" + result.uuid;
//                    Swal.fire({
//                        title: 'Berhasil',
//                        text: result.message,
//                        icon: 'success',
...
//                    });
    ...
    $(document).on("click", "button.btn-add-jenis-bank-nota", function (e) {
        ...
        $.get("/banknota/listbn" + '/' + $(this).val() + '/spt', function (data) {
            $('#company-label').show();
