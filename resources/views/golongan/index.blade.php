    ...
    <div class="col-12">
        <div class="card card-primary card-outline">
            <div class="card-header">
                <h3 class="card-title">Golongan <small>list</small></h3>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    <label for="golongan_code" class="col-sm-2 control-label">Nama</label>
                    <div class="col-sm-12">
                        <input type="text" class="form-control" id="golongan_code" name="golongan_code" placeholder="Enter Code" maxlength="256">
                        <div class="invalid-feedback invalid-golongan_code"></div>
                    </div>
                </div>
                <div class="form-group">
...
<script>
    $(function () {
        ...
        var _dataTable = $('.data-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('golongan.fetch') }}",
            columns: [
                {data: 'id', name: 'id'},
                {data: 'golongan_code', name: 'golongan_code'},
                {data: 'name', name: 'name'},
        ...
        $('body').on('click', '.action-edit', function () {
            ...
            var id = $(this).data('id');
            $.get("golongan" + '/' + id + '/edit', function (data) {
                $('#modelHeading').html("Edit Golongan");
                $('#ajaxModel').modal('show');
                $('#id').val(data.id);
                $('#golongan_code').val(data.golongan_code);
                $('#golongan_code').prop('readonly', false);
                $('#name').val(data.name);
        ...
        $('body').on('click', '.action-view', function () {
            ...
            var id = $(this).data('id');
            $.get("golongan" + '/' + id + '/edit', function (data) {
                $('#modelHeading').html("View Golongan");
                $('#ajaxModel').modal('show');
                $('#id').val(data.id);
                $('#golongan_code').val(data.golongan_code);
                $('#golongan_code').prop('readonly', true);
                $('#name').val(data.name);
        ...
        $('body').on('click', '.btn-action-add', function () {
            $('#modelHeading').html("Add Golongan");
            $('#ajaxModel').modal('show');
            $('#id').val('');
            $('button.btn-submit-update').show();
            $('button.btn-save').html('<i class="far fa-save"></i> Simpan');
