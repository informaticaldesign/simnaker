    ...
    <div class="col-12">
        <div class="card card-primary card-outline">
            <div class="card-header">
                <h3 class="card-title">Sektor <small>list</small></h3>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    <label for="sektor_code" class="col-sm-12 control-label">Kode Sektor</label>
                    <div class="col-sm-12">
                        <input type="text" class="form-control" name="sektor_code" placeholder="Enter Code" maxlength="18">
                        <div class="invalid-feedback invalid-sektor_code"></div>
                    </div>
                </div>
                <div class="form-group">
                    <label for="name" class="col-sm-12 control-label">Nama Sektor</label>
                    <div class="col-sm-12">
...
<script>
    $(function () {
        ...
        var _dataTable = $('.data-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('admin.sektor.fetch') }}",
            columns: [
                {data: 'id', name: 'id'},
                {data: 'sektor_code', name: 'sektor_code'},
                {data: 'name', name: 'name'},
        ...
        $('body').on('click', '.action-delete', function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "DELETE",
                        url: "{{ url('admin/sektor/destroy') }}" + "/" + id,
                        dataType: 'JSON',
                        data: {
                            'id': id,
                        },
        ...
        $('body').on('click', '.action-edit', function () {
            ...
            var id = $(this).data('id');
            $.get("/admin/sektor" + '/' + id + '/edit', function (data) {
                $('#modelHeading').html("Edit Sektor");
        ...
        $('body').on('click', '.btn-action-add', function () {
            $('#modelHeading').html("Tambah Sektor");
            $('#ajaxModel').modal('show');
            $('#id').val('');
            $('button.btn-submit-update').show();
            $('button.btn-save').html('<i class="far fa-save"></i> Simpan');
