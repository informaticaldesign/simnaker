    ...
    <div class="col-12">
        <div class="card card-primary card-outline">
            <div class="card-header">
                <h3 class="card-title">Pangkat <small>list</small></h3>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    <label for="pangkat_code" class="col-sm-2 control-label">Code</label>
                    <div class="col-sm-12">
                        <input type="text" class="form-control" id="pangkat_code" name="pangkat_code" placeholder="Enter Code" maxlength="256">
                        <div class="invalid-feedback invalid-pangkat_code"></div>
                    </div>
                </div>
                <div class="form-group">
...
<script>
    $(function () {
        ...
        var _dataTable = $('.data-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('pangkat.fetch') }}",
            columns: [
                {data: 'id', name: 'id'},
                {data: 'pangkat_code', name: 'pangkat_code'},
                {data: 'name', name: 'name'},
        ...
        $('body').on('click', '.action-edit', function () {
            ...
            $.get("pangkat" + '/' + id + '/edit', function (data) {
                $('#modelHeading').html("Edit Pangkat");
                $('#ajaxModel').modal('show');
                $('#id').val(data.id);
                $('#pangkat_code').val(data.pangkat_code);
                $('#pangkat_code').prop('readonly', false);
                $('#name').val(data.name);
        ...
        $('body').on('click', '.action-view', function () {
            ...
            var id = $(this).data('id');
            $.get("pangkat" + '/' + id + '/edit', function (data) {
                $('#modelHeading').html("View Pangkat");
                $('#ajaxModel').modal('show');
                $('#id').val(data.id);
                $('#pangkat_code').val(data.pangkat_code);
                $('#pangkat_code').prop('readonly', true);
                $('#name').val(data.name);
