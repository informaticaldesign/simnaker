    ...
    <div class="col-12">
        <div class="card card-primary card-outline">
            <div class="card-header">
                <h3 class="card-title">Bidang Usaha <small>list</small></h3>
...
<script>
    $(function () {
        ...
        $('body').on('click', '.action-delete', function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "DELETE",
                        url: "{{ url('bidangusaha/destroy') }}" + "/" + id,
                        dataType: 'JSON',
                        data: {
                            'id': id,
                        },
        ...
        $('body').on('click', '.action-edit', function () {
            ...
            var id = $(this).data('id');
            $.get("bidangusaha" + '/' + id + '/edit', function (data) {
        ...
        $('body').on('click', '.action-view', function () {
            ...
            var id = $(this).data('id');
            $.get("bidangusaha" + '/' + id + '/edit', function (data) {
                $('#modelHeading').html("View Bidang Usaha");
                $('#ajaxModel').modal('show');
                $('#id').val(data.id);
                $('#name').val(data.name);
        ...
        $('body').on('click', '.btn-action-add', function () {
            $('#modelHeading').html("Add Bidang Usaha");
            $('#ajaxModel').modal('show');
            $('#id').val('');
            $('button.btn-submit-update').show();
            $('button.btn-save').html('<i class="far fa-save"></i> Simpan');
