    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">
                    6. PROSES PERSETUJUAN DAN DELEGASI KE UPT
                </h3>
            </div>
            ...
            <div class="card-body">
                ...
                <p class="card-text">
                    Pilih Kepala UPT untuk delegasi uji kegiatan pemeriksaan dan pengawasan
                </p>
                {{--<table id="dtBasicExample"  class="table table-striped table-bordered table-sm">
                    <thead>
                        <tr class="blockHeader">
                            <th width="40%">
                                @if($data->status == 'terkirim')<input class="alignTop" type="checkbox" id="module_select_all">&nbsp; @endif Nama Pegawai
                            </th>
                            <th width="15%">
                                NIP
                            </th>
                    ...
                    <tr>
                        <td> @if($data->status == 'terkirim')<input module_id="{{ $module->id }}" class="module_checkb" type="checkbox" name="pegawai[{{$module->id}}]" value="{{$module->id}}" id="pegawai_{{$module->id}}">&nbsp;@endif  {{ $module->name }}</td>
                        <td>{{ $module->nip }}</td>
                ...
                <div class="row mb-2 dropdown-upt hide">
                    <div class="col-lg-6">
                        <p>Unit Pelayanan Teknis</p>
                        <div class="form-group">
                            <select class="form-control" id="biodata_upt_id" name="biodata_upt_id">
                                <option value="" selected disabled>Pilih UPT</option>@foreach($upt as $key => $val)
                                <option value="{{ $val->biodata_id }}">{{ $val->name }} (KEPALA {{ $val->upt_name }})</option>@endforeach
                            </select>
                            <div class="invalid-feedback invalid-biodata_upt_id"></div>
                        </div>
                        <!--<button class="btn btn-outline-lite py-0 add_new_frm_field_btn_k3"><i class="fas fa-plus add_icon"></i> Tambah</button>-->
                    </div>
                </div>
                <div class="row mb-2 reason">
                    <div class="col-lg-6">
                        <p>Catatan</p>
                        <div class="form-group">
                ...
                <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                    ...
                    <button class="btn btn-success btn-action-next">Delegasi Ke UPT&nbsp;<i class="fa fa-paper-plane"></i></button>
                </div>
            </div>
            {{ Form::close() }}
        </div>
    ...
    $('button.btn-action-next').click(function (e) {
        ...
        $.ajax({
            ...
            error: function (err) {
                ...
                $('button.btn-action-next').html('Delegasi Ke UPT&nbsp;<i class="fa fa-paper-plane"></i>');
