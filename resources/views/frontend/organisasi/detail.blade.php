...
<div class="container search-header">
    <section class="content">
        <span class="search-headline">{{ $organization->name }}</span>
            ...
            <section class="search-results">
                <div class="search-results-header">
                    <h2 class="section-title">
                        @if( $total > 0 )
                        {{ $total }}  Dataset ditemukan di {{ $organization->name }}
                        @else
                        Tidak ada dataset ditemukan di {{ $organization->name }}
                        @endif
                    </h2>
                ...
                <div class="text-center mt-3">
                    <img src="{{ url('/images/frontend/data-not-found.svg') }}" width="55%" class="rounded" alt="Data not found">
                </div>
                @endif
                @if($totalpage > 1)
                <div class="text-center">
                    <div class="pagination pagination-centered">
                        <ul> 
                            @if($page > 1)
                            <li><a href="{{ url('/organisasi') }}/{{ $slug }}?page={{ $page-1 }}">«</a></li>
                            @endif

                            @if($page < 5)
                            @for($x=1;$x<= ($page+2);$x++)
                            <li class="{{ $page==$x?'active':''}}"><a href="{{ url('/organisasi') }}/{{ $slug }}?page={{ $x }}">{{ $x }}</a></li> 
                            @endfor
                            <li class="disabled"><a href="#">...</a></li>
                            <li class="{{ $page==$totalpage?'active':''}}"><a href="{{ url('/organisasi') }}/{{ $slug }}?page={{ $totalpage }}">{{ $totalpage }}</a></li>
                            @endif

                            @if($page >= 5)
                            <li class="{{ $page==1?'active':''}}"><a href="{{ url('/organisasi') }}/{{ $slug }}?page=1">1</a></li> 
                            <li class="disabled"><a href="#">...</a></li>
