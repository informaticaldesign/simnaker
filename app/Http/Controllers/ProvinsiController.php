...
class ProvinsiController extends Controller {
    ...
    public function edit($id) {
        $Users = Provinsi::find($id);
        return response()->json($Users);
    }

    public function update(Request $request, Provinsi $datapost) {
        $validator = \Validator::make($request->all(), [
                    'name' => ['required', 'string', 'max:255'],
                    'prov_code' => ['required', 'string', 'max:18', 'unique:m_provinsi,prov_code,' . $request->id],
        ]);
        ...
        if ($request->id) {
            ...
            Provinsi::where('id', $request->id)->update($input);
            $status = 'update';
        } else {
