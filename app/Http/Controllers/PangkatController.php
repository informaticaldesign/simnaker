...
class PangkatController extends Controller {
                            ...
                            ->filter(function ($instance) use ($request) {
                                if (!empty($request->get('search'))) {
                                    $instance->where(function($w) use($request) {
                                        ...
                                        $w->orWhere('pangkat_code', 'LIKE', "%" . Str::lower($search['value']) . "%");
    ...
    public function edit($id) {
        $Users = Pangkat::find($id);
        return response()->json($Users);
    }

    public function update(Request $request, Pangkat $datapost) {
        $validator = \Validator::make($request->all(), [
                    'name' => ['required', 'string', 'max:255'],
                    'pangkat_code' => ['required', 'string', 'max:18', 'unique:m_pangkat,pangkat_code,' . $request->id],
        ]);
        ...
        $input = [
            'name' => $request->name,
            'pangkat_code' => $request->pangkat_code,
        ];
        if ($request->id) {
            $input['updated_at'] = date('Y-m-d H:i:s');
            $input['updated_by'] = $user->id;
            Pangkat::where('id', $request->id)->update($input);
            $status = 'update';
        } else {
