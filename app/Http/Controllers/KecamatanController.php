...
class KecamatanController extends Controller {
    ...
    public function fetch(Request $request) {
        if ($request->ajax()) {
            $data = Kecamatan::select(['m_kecamatan.*', 'm_provinsi.name AS prov_name', 'm_kota.name AS city_name'])
    ...
    public function edit($id) {
        $Users = Kecamatan::select(['m_kecamatan.*', 'm_provinsi.prov_code', 'm_kota.city_code'])
                        ->leftJoin('m_kota', 'm_kota.city_code', '=', 'm_kecamatan.city_code')
                        ->leftJoin('m_provinsi', 'm_kota.prov_code', '=', 'm_provinsi.prov_code')
                        ->where('m_kecamatan.id', $id)->first();
        return response()->json($Users);
    }

    public function update(Request $request, Kecamatan $datapost) {
        $validator = \Validator::make($request->all(), [
                    'name' => ['required', 'string', 'max:255'],
                    'prov_code' => ['required', 'string', 'exists:m_provinsi,prov_code'],
                    'city_code' => ['required', 'string', 'exists:m_kota,city_code'],
                    'kec_code' => ['required', 'string', 'max:18', 'unique:m_kecamatan,kec_code,' . $request->id],
        ]);
        ...
        if ($request->id) {
            ...
            Kecamatan::where('id', $request->id)->update($input);
            $status = 'update';
        } else {
    ...
    public function combo($code) {
        $cities = \App\Models\Kecamatan::where('city_code', $code)
                ->pluck('name', 'kec_code');
