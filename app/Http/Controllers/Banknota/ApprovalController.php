...
class ApprovalController extends Controller {
    ...
    public function index() {
        return View('banknota.approval.index');
    }

    public function fetch(Request $request) {
        if ($request->ajax()) {
            $data = Banknota::select('*')->where('status', 'send');
            return DataTables::of($data)
                            ...
                            ->addColumn('status', function($row) {
                                ...
                                if ($row->status == 'open') {
                                    ...
                                    $btn = '<small class="badge badge-info">Menunggu Persetujuan</small>';
                            ...
                            ->addColumn('action', function($row) {
                                ...
                                $btn .= '<a href="' . url('banknota/approval/view' . '/' . $row->uuid) . '" data-toggle="tooltip"  data-id="' . $row->id . '" data-original-title="View" class="action-view btn btn-primary btn-sm"><i class="fas fa-eye"></i></a> ';
                                return $btn;
                            })
    ...
    public function create() {
        ...
        $kotas = explode(",", $kotas);
        return View('banknota.approval.form', [
            'spt' => $spt,
            'sifat' => $sifat,
            'jenis' => $jenis,
            'kotas' => $kotas,
    ...
    public function edit($slug) {
        ...
        return View('banknota.approval.edit', [
            'spt' => $spt,
            'sifat' => $sifat,
            'jenis' => $jenis,
            'kotas' => $kotas,
    ...
    public function preview($slug) {
        ...
        return View('banknota.approval.preview', [
            'banknota' => $banknota,
            'jenis' => $relbank
        ]);
    }
    ...
    public function view($slug) {
        $banknota = Banknota::where('uuid', $slug)->first();
        if ($banknota->version == 1) {
            $relbank = \App\Models\Banknota\Relbank::select(['id', 'description'])->where('banknota_id', $banknota->id)->get();
            return View('banknota.approval.view', [
                'banknota' => $banknota,
                'jenis' => $relbank
            ]);
        } elseif ($banknota->version == 2) {
            $banknota = Banknota::where('uuid', $slug)->first();
            $relbank = \App\Models\Banknota\Reltembusan::select(['id', 'description'])->where('banknota_id', $banknota->id)->get();
            return View('banknota.approval.viewii', [
                'banknota' => $banknota,
                'jenis' => $relbank
            ]);
        }
    ...
    public function send(Request $request) {
        ...
        Banknota::where('uuid', $request->id)->update($input);
        return response()->json([
                    'success' => true,
                    'message' => 'Banknota Berhasil disetujui',
        ]);
    }
    ...
    public function cetak(Request $request) {
        ...
        $relbank = \App\Models\Banknota\Relbank::select(['id', 'description'])->where('banknota_id', $banknota->id)->get();
        $pdf = PDF::loadView('banknota.approval.cetak', [
                    'banknota' => $banknota,
                    'jenis' => $relbank
                        ], [], $configs);
