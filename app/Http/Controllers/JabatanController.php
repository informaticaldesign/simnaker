...
class JabatanController extends Controller {
    ...
    public function edit($id) {
        $Users = Jabatan::find($id);
        return response()->json($Users);
    }

    public function update(Request $request, Jabatan $datapost) {
        $validator = \Validator::make($request->all(), [
                    'name' => ['required', 'string', 'max:255'],
                    'jabatan_code' => ['required', 'string', 'max:18', 'unique:m_jabatan,jabatan_code,' . $request->id],
        ]);
        ...
        if ($request->id) {
            ...
            Jabatan::where('id', $request->id)->update($input);
            $status = 'update';
        } else {
